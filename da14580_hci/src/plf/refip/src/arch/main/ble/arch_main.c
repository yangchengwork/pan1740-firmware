 ...
 * History:
 *
 * Version 0.90(alpha) 30.04.2015
 *
 *  - enable last part of jumptable, as per 14581, including HCI trasks
 *  - uart.c is needed to replace non-working rom functions
 *  - specific da14580/581 mixed scatter file is rqeuired
 *  - changed patch addresses for da14580 ROM functions
 *  - define __DA14581__ must be added in some files only
 *
 */
...
/* 
 * angelo
 * we have to act here as da14581
 */
#define __DA14581__
...
#ifdef __DA14581__
	(const uint32_t *) llm_rd_local_supp_feats_cmd_handler,  // mandatory for hciconfig hci0 up
	(const uint32_t *) lld_adv_start_patch,
	(const uint32_t *) llc_con_update_req_ind,
#endif
...
void patch_func (void)
{
	/*
	 * angelo
	 * here we have to use svc values from da14580
	 */

#ifdef __DA14581__ 

	//0x00024f23 lld_adv_start
	SetWord32(PATCH_ADDR0_REG, 0x00024f22);
	SetWord32(PATCH_DATA0_REG, 0xdf014770); // lld_adv_start svc 1 (+ enabling of interrupts)

	// 0x00027bcf OK hardly found on attempts from 27bb4 to 27c04 step 4
	SetWord32(PATCH_ADDR1_REG, 0x00027bcc); 
	SetWord32(PATCH_DATA1_REG, 0xdf00b662); // llm_rd_local_supp_feats_cmd_handler svc 0

	// 0x00027b97 OK found subctracting fixed offs from above 581 offset
	SetWord32(PATCH_ADDR2_REG, 0x00027b94); // llm_rd_local_ver_info_cmd_handler
	SetWord32(PATCH_DATA2_REG, 0x80c121d2); // patch manufacturer ID

	// 0x00022e29
	SetWord32(PATCH_ADDR3_REG, 0x00022e26); // llc_version_ind_pdu_send
	SetWord32(PATCH_DATA3_REG, 0x80d020d2); // patch manufacturer ID
	
	// 0x000233bf  llc_con_update_req_ind
	SetWord32(PATCH_ADDR4_REG, 0x000233bc);
	SetWord32(PATCH_DATA4_REG, 0xdf02bdf8); //llc_con_update_req_ind svc 2

#endif

#if 0
	//0x0002a32b l2cc_pdu_recv_ind_handler (atts)
	SetWord32(PATCH_ADDR4_REG, 0x0002a328);
	SetWord32(PATCH_DATA4_REG, 0xdf02bdf8); //l2cc_pdu_recv_ind_handler svc 2
	
	//0x000233bf  llc_con_update_req_ind
	SetWord32(PATCH_ADDR5_REG, 0x000233bc);
	SetWord32(PATCH_DATA5_REG, 0xdf03e7f5); //llc_con_update_req_ind svc 3

	//0x0002341b  llc_ch_map_req_ind
	SetWord32(PATCH_ADDR6_REG, 0x00023418);
	SetWord32(PATCH_DATA6_REG, 0xdf04b089); //llc_ch_map_req_ind svc 4

	//0x00030cef gapm_adv_op_sanity
	SetWord32(PATCH_ADDR7_REG, 0x00030cec);
	SetWord32(PATCH_DATA7_REG, 0xdf05bdf8); //gapm_adv_op_sanity svc 5
	...
	//SetWord32(PATCH_DATA4_REG, 0xdf04b089); // angelo, trying
	
	/* MANDATORY */
	SetWord32(PATCH_ADDR0_REG, 0x00027bcc); 
	SetWord32(PATCH_DATA0_REG, 0xdf00b662); // llm_rd_local_supp_feats_cmd_handler svc 0
	
	SetWord32(PATCH_ADDR1_REG, 0x0002a328);
	SetWord32(PATCH_DATA1_REG, 0xdf014770); // l2cc_pdu_recv_ind_handler svc 1

	//0x00024f23 lld_adv_start
	SetWord32(PATCH_ADDR2_REG, 0x00032794);
	SetWord32(PATCH_DATA2_REG, 0xdf02bdf8); // cmp_abs_time svc 0 (+ enabling of interrupts)
	//0x00032795 cmp_abs_time
	//SetWord32(PATCH_ADDR0_REG, 0x00032794);
	//SetWord32(PATCH_DATA0_REG, 0xdf00b662); //cmp_abs_time svc 0 (+ enabling of interrupts)
	...
	//0x0002d485  smpc_pairing_cfm_handler
	SetWord32(PATCH_ADDR4_REG, 0x0002d484);
	//SetWord32(PATCH_DATA4_REG, 0xdf04b089); //smpc_pairing_cfm_handler svc 4
	SetWord32(PATCH_DATA4_REG, 0xb089df04);
